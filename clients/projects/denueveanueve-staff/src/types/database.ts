// =============================================================================
// Tipos de base de datos — Salón OS (proyecto Supabase: jztoyekixcziaicrnlce)
// -----------------------------------------------------------------------------
// Generado con:
//   supabase gen types typescript --project-id jztoyekixcziaicrnlce
// Regenerado el 2026-07-18 (HAT3X-023) al re-apuntar la app a la BD de Salón OS.
//
// Sustituye al esquema antiguo (proyecto descartado cpocwvedqlxtwazwoyfn), que
// definía tablas como `staff_members`, `employee_schedules`, `user_roles`,
// `audit_logs` y una tabla `customers` con `first_name`/`last_name`/`status`.
//
// COMPAT TRANSITORIA:
//   El esquema real de Salón OS ya NO contiene esas tablas/columnas heredadas,
//   pero varias pantallas todavía las consultan (Customers, VerifyCustomer,
//   Dashboard, History, AdminEmployees, AdminEmployeeCalendar, hooks de staff y
//   auth). Para no romper la compilación mientras se migra ese código de app en
//   subtareas posteriores, se conservan aquí como "compat transitoria" y están
//   marcadas con el comentario `// [compat]`. Deben eliminarse cuando la app se
//   haya migrado por completo al esquema de Salón OS.
//
//   Regenerar (reemplaza TODO menos los bloques `// [compat]`):
//   npx supabase gen types typescript --project-id jztoyekixcziaicrnlce
// =============================================================================
...
export type Database = {
  ...
  public: {
    Tables: {
      // ======================================================================
      // [compat] Tablas del esquema antiguo (proyecto descartado) que todavía
      // consulta la app sin cast `as any`. No existen en la BD de Salón OS.
      // Se conservan para mantener la compilación TS hasta migrar el código.
      // Eliminar cuando Dashboard/History/AdminEmployees/AdminEmployeeCalendar/
      // useStaffMember/useScheduleEntries/auth se migren al esquema de Salón OS.
      // ======================================================================
      audit_logs: {
        Row: {
          id: string
      ...
      customers: {
        Row: {
          ...
          email: string | null
          full_name: string
          // [compat] columnas del esquema antiguo aún leídas por Customers.tsx /
          // VerifyCustomer.tsx. No existen en la BD de Salón OS; eliminar tras
          // migrar esas pantallas a `full_name` / `phone_e164`.
