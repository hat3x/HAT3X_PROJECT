...
describe("runNueva", () => {
  ...
  it("creates a task and returns formatted output", async () => {
    const output = await runNueva({ order: "Web para test CLI", mode: undefined, clientId: undefined, skipAnalysis: true })
  ...
  it("respects explicit mode flag", async () => {
    const output = await runNueva({ order: "Test autopilot", mode: "autopilot", clientId: undefined, skipAnalysis: true })
    const m = output.match(/HAT3X-\d{3}/)
    if (m?.[0] != null) ids.push(m[0])
