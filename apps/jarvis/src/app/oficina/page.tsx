...
export default function OficinaPage() {
  const { agents, events, verticalByAgent } = useOfficeState();
    ...
    <main style={{ maxWidth: 1100, margin: '0 auto', background: '#f3f4f6', minHeight: '100vh' }}>
      <header style={{ padding: '16px 16px 0' }}>
        ...
        <p style={{ fontSize: 13, color: '#6b7280' }}>
          🟢 {working} trabajando · 🔵 {meeting} en reunión · 🔴 {blocked} bloqueados · ⚪ {idle} disponibles · plantilla de {agents.length}
        </p>
      </header>
      <OfficeFloor agents={agents} verticalByAgent={verticalByAgent} onSelect={setSelected} />
      {selected !== null && (
        <AgentPanel
