            ...
            >
              ...
              <div className="space-y-3">
                ...
                <div aria-live="polite" className="min-h-[3rem]">
                  {availabilityQuery.isLoading && <SlotSkeleton />}
                  ...
                  {availabilityQuery.isSuccess &&
                    availabilityQuery.data.length === 0 && (
                      <EmptyState
                        icon={CalendarX}
                        message="No hay horas libres este día. Prueba con otra fecha."
                      />
                    )}
                  ...
                  {availabilityQuery.isSuccess &&
                    availabilityQuery.data.length > 0 && (
                      <div className="space-y-4">
                        {groupSlots(availabilityQuery.data, timeZone).map(
                          (group) => {
                            const GroupIcon =
                              SLOT_GROUP_ICON[group.label] ?? Clock;
                            return (
                                ...
                                <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
                                  {group.slots.map((available) => (
                                    <button
                                      ...
                                      className="flex h-11 items-center justify-center rounded-lg border border-border/70 bg-card text-sm font-medium tabular-nums shadow-xs transition-all duration-200 ease-apple-out hover:-translate-y-0.5 hover:border-primary hover:bg-accent hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 active:scale-95"
                                    >
                                      {formatSlotTime(
                                        available.startsAt,
                                        timeZone,
...
/** Esqueleto de carga para la rejilla de huecos. */
